...
void susu_http_object::get_request_and_header(int fd)
{
    ...
    int numchars = get_a_line(fd,buffer, sizeof(buffer));//获得http头部
    if(numchars < 4)
    {
        //头部太小，不处理，直接返回
        return ;
    ...
    if(space_count != 2)
    {
        //不满足有且只有2个空格，不符合格式，不作处理，直接返回
        return;
    }

    //按空格划分字符串,分为method url version
    sscanf(buffer,"%[^ ] %[^ ] %[^'\n']",method,url,version);
    string METHOD = method;
    string URL = url;
    string VERSION = version;
    temp_log_str = std::move(METHOD+ " " + URL+ " " + VERSION + " socket is "+std::__cxx11::to_string(fd));

    log->print_a_line(2,temp_log_str);
...
{
    memset(method,0,line_limit);
    memset(url,0,line_limit);
    memset(version,0,line_limit);
...
bool susu_http_object::check_all_space(char* str) //检查一行是否全为空格
{
    bool ret = true;
...
void susu_http_object::process(int fd)   //根据之前读到的信息，对这个描述符进行处理
{
    ...
    if(strcmp(method,"POST") == 0)//POST
    {
        ...
        //string true_target_url = init_param->get_string_value("root_path")+target_url;
        //检查这个脚本是否存在
    }
    else if(strcmp(method,"GET") == 0)//GET
    {
        //不在此处作处理，而仅仅是转发给对应脚本
        run_get(fd,"get.lua",url,http_head_kv["Accept"].c_str());
    }
    else//unimplemented
    {
        //unimplemented(fd);
...
void susu_http_object::run_get(int fd,char* script_name,char* url,const char* type)    //执行脚本处理get请求
{
    // root_path+url将组成真正的url，这个url可能是一个文件 或者 一个路径，需要在脚本内作判断
    susu_init_param* init_param = susu_init_param::get_Init_Param();
    Log_Printer* log = Log_Printer::get_Log_Printer();
    string temp_log_str;
    
    char exe_cmd[2048];
    //sprintf(exe_cmd,"lua %s/%s %s%s %s\0",init_param->get_string_value("root_path").c_str(),script_name,init_param->get_string_value("root_path").c_str(),url,type);
    sprintf(exe_cmd,"lua ./%s %s%s %s\0",script_name,init_param->get_string_value("root_path").c_str(),url,type);

    string EXE_CMD = exe_cmd;
    temp_log_str = EXE_CMD + "socket is "+std::__cxx11::to_string(fd);
    //log->print_a_line(2,temp_log_str);

    char ret[1024*4+32];//文件缓冲区
    FILE *fp;//文件流
    fp = popen(exe_cmd,"r");
            
    int size = fread(ret,1,1024*4,fp);
    while(size > 0)
    {
        send(fd, ret, size, 0);
        //printf("%s",ret);
        size = fread(ret,1,1024*4,fp);
    }
    pclose(fp);
}

void susu_http_object::run_get_text(int fd,char* script_name,char* filename,char* type)    //执行get_text(filename,type)脚本
{
    susu_init_param* init_param = susu_init_param::get_Init_Param();
    Log_Printer* log = Log_Printer::get_Log_Printer();
    string temp_log_str;
    ...
    sprintf(exe_cmd,"lua ./%s %s/%s %s\0",script_name,init_param->get_string_value("root_path").c_str(),filename,type);
